...
export function LendersGrid() {
  ...
  return (
    <section id="preteurs" className="py-[clamp(4rem,8vw,7rem)] surface-cream border-t border-[color:var(--color-border)] relative overflow-hidden">
      ...
      <span
        aria-hidden="true"
        className="absolute -top-12 left-1/2 -translate-x-1/2 font-[family-name:var(--font-editorial)]  text-[color:var(--color-taupe)]/10 text-[24rem] leading-none pointer-events-none select-none"
      >
        9
      </span>
      ...
      <Container size="xl" className="relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[clamp(1.75rem,2.5vw,2.25rem)]" style={{ perspective: "1200px" }}>
          {lenders.map((l, idx) => (
            ...
            <article
              className="group relative halo-glow bg-[color:var(--color-surface)] border border-[color:var(--color-taupe)]/40 p-8 transition-all duration-500 hover:border-[color:var(--color-orange)]/70 hover:shadow-[0_18px_40px_-22px_rgba(16,34,61,0.28)] flex flex-col h-full"
            >
              {/* Numéro filigrane (01-09) */}
              <span
                aria-hidden="true"
                className="absolute top-4 right-5 font-[family-name:var(--font-editorial)]  text-[color:var(--color-taupe)]/30 text-4xl leading-none pointer-events-none select-none transition-colors duration-500 group-hover:text-[color:var(--color-bronze)]/40"
              >
              ...
              {/* Nom */}
              <h3 className="font-[family-name:var(--font-display)] font-bold text-[color:var(--color-navy-deep)] text-base uppercase tracking-[0.04em] mb-3 leading-snug text-balance">
                {l.name}
              </h3>
